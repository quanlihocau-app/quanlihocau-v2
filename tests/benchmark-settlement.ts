...
async function runBenchmark() {
  ...
  console.log(`Found session: ${session.id}, status: ${session.status}, lakeId: ${lakeId}`);

  const invoiceId = session.invoices[0]?.id;
  console.log(`Associated invoiceId: ${invoiceId}`);

  const timings = {
    sessionQuery: [] as number[],
    invoiceFirst: [] as number[],
    invoiceLines: [] as number[],
    invoicePayments: [] as number[],
    invoiceRequeryFinal: [] as number[],
    sessionRequery: [] as number[],
    combinedInvoiceWithRelations: [] as number[]
  };
  ...
  for (let i = 0; i < ITERATIONS; i++) {
    ...
    await prisma.fishingSession.findFirst({
      where: { id: session.id, lakeId: lakeId },
      include: {
        hutLinks: { select: { hutId: true } },
        customer: { select: { name: true, phoneNormalized: true } },
        package: {
    ...
    const inv = await prisma.invoice.findFirst({
      where: { fishingSessionId: session.id, lakeId: lakeId }
    });
